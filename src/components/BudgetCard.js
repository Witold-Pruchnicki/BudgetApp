...
export default function BudgetCard(props) {
    const {name, amount, max, gray, onAddExpenseClick,hideButtons, onViewExpensesClick} = props
    const classNames = []
    if (amount > max) {
            ...
            <Card.Body>
                <Card.Title className="d-flex justify-content-between align-items-baseline fw-normal mb-3">
                    ...
                    <div className="d-flex align-items-baseline">{currencyFormat.format(amount)}
                        {max && (<span className="text-muted fs-6 ms-1">/ {currencyFormat.format(max)}</span>)}</div>
                </Card.Title>
                {max && (
                    <ProgressBar className="rounded-pill" variant={getProgressBarVariant(amount, max)} min={0} max={max}
                                 now={amount}/>
                )}
